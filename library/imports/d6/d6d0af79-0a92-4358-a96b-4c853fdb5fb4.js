"use strict";
cc._RF.push(module, 'd6d0a95CpJDWKlrTIU/21+0', 'js_home');
// JS/js_home.js
...
cc.Class({
  ...
  start: function start() {
    ...
    this.cardRegion.init(); //使用数据更新数据区域

    this.dataRegion.updateInfo(this.information.dataInfo); //往卡牌区域依次加入（一张）卡牌

    this.cardRegion.push(this.information.card_1);
    this.cardRegion.push(this.information.card_2);
    this.cardRegion.push(this.information.card_3); //显示数据和卡牌
    //this.dataRegion.show();

    this.cardRegion.show(); //设置按键监听事件 //还不清楚这么用监听事件传递参数，只能这样写
